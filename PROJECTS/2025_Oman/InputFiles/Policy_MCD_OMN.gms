...
* 3. Define acting regions
    ...
    actingReg(r) $ KAZ(r) = NO ;

***NEW
    actingReg(r) $ WHD(r) = NO ;


$iftheni.dtadj  %SimGroup%=="DelayTran"
    actingReg(r) $ (allMCD(r) or AFR(r) ) = NO ; !! or IND(r) or REU(r)
    EmiTotTgt(r,"%YrFinTgt%") $ (WHD(r) or AFR(r)) = EmiTotCal(r,"DelayTran","%YrFinTgt%") + 750 ;
*    EmiTotTgt(r,"%YrFinTgt%") $ (AFR(r)) = EmiTotCal(r,"DelayTran","%YrFinTgt%") + 1000 ;
$endif.dtadj

$iftheni.nzadj  %SimGroup%=="NetZero"
    actingReg(r) $ (allMCD(r)  ) = NO ;

$endif.nzadj

$ifi  %SimName%=="SimNZ"    actingReg(r) $ OMN(r) = YES ;
...
******************************************************************************************************************************************
***HRR: this creates a linear interpolation between YearAntePolicy and YrFinTgt
*** But it doesn't work for NDC when YearEndofSim = 2040, as most NDC reductions are by 2030 and then remain relatively flat after!

$iftheni.NDC not %SimGroup%=="NDC"
...
$else.NDC
* EmiTotTgt(r,"%YrFinTgt%") $ EmiTotCal(r,"NDCs","%YrFinTgt%") = EmiTotCal(r,"NDCs","%YrFinTgt%") ;
* Define emitot target for %YrFinTgt% 
    loop(t $ (t.val ge %YearAntePolicy%),
        emiCapFull.fx(rq,t) $ IfCap(rq)
            = sum(mapr(rq,r), EmiTotCal(r,"NDCs",t) $ EmiTotCal(r,"NDCs",t)) * cScale ;
    );
* Linear interpolation from %YearPolicyStart" to %YrFinTg
$endif.NDC
...
GovBalance(r)  = 0 ;  !! default closure using direct taxes
...
* Using NGFS elymix for non-MCD countries for NetZero scenario
** issue that elyMix in gdx has only G20 regions, not MCD regions, so only USA, CHN, IND, REU ==EUR
*** Will need to do the proper aggregation in an NGFS project

parameter Elymix_NGFS(r,a,NGFS_sce,t) "Elymix from NGFS used in MCDagg for non-MCD regions" ;

    EXECUTE_LOAD '%ExtDir%/ExternalScenarios/%GroupName%/ElyMixNGFS_MCD.gdx', Elymix_NGFS ;
    ElyMixCal(r,a,NGFS_sce,t) $ NGFSreg(r)= ElyMix_NGFS(r,a,NGFS_sce,t) ;

$$endif.netzero

*****************
*** Include NGFS values for elymix in Delayed transition
$ontext
$$iftheni.delaytran %SimGroup%=="DelayTran"
    $$batinclude "%iFilesDir%\NGFS_forMCDagg.gms" 


* Using NGFS elymix for non-MCD countries for DT scenario
** issue that elyMix in gdx has only G20 regions, not MCD regions, so only USA, CHN, IND, REU ==EUR
*** Will need to do the proper aggregation in an NGFS project
...
parameter Elymix_NGFS(r,a,NGFS_sce,t) "Elymix from NGFS used in MCDagg for non-MCD regions" ;
    ...
    EXECUTE_LOAD '%ExtDir%/ExternalScenarios/%GroupName%/ElyMixNGFS_MCD.gdx', Elymix_NGFS ;
    ElyMixCal(r,a,"DelayTran",t) $ NGFSreg(r)= ElyMix_NGFS(r,a,"NDCs",t) ;
...
*******************************************************************************************************************
*** Oman: Elimination of FFS between 2025 and 2030
...
*** Different recycling options for Oman
* 1. Makes 50% of revenue increase go to transfers and 50% to savings (rsg)
if((VarFlag eq 101) , 
    GovBalance(r) $ (OMN(r)) = 1 ;  !! makes rsg increase (not revenue neutral)
        execute_load "%BauFile%.gdx", ygov_bau=ygov.l ;
***Needs to run SimFFSup1 before to get the changes in ygov when FFSR and 100% rsg !        
        execute_load "%oDir%\SimFFSup1.gdx", ygov_sim=ygov.l
    if((year ge 2025 and (year le 2030)),
        trg.fx(r,tsim) $ OMN(r) = 0.5 * ygov0(r,"itax") * (ygov_sim(r,"itax",tsim) - ygov_bau(r,"itax",tsim)) ;
        trg.fx(r,tsim) $ OMN(r) = trg.l(r,tsim) + (ygov0(r,"ctax") * (ygov.l(r,"ctax",tsim-1) - ygov_bau(r,"ctax",tsim-1))) ;        
    );

) ;

* 2. Makes 75% of revenue increase go to transfers and 25% to savings (rsg)
if((VarFlag ge 102)  , 
    GovBalance(r) $ (OMN(r)) = 1 ;  !! makes rsg increase (not revenue neutral)
        execute_load "%BauFile%.gdx", ygov_bau=ygov.l ;
***Needs to run SimFFSup1 before to get the changes in ygov when FFSR and 100% rsg !        
        execute_load "%oDir%\SimFFSup1.gdx", ygov_sim=ygov.l
    if((year ge 2025 and (year le 2030)),
        trg.fx(r,tsim) $ OMN(r) = 0.25 * ygov0(r,"itax") * (ygov_sim(r,"itax",tsim) - ygov_bau(r,"itax",tsim)) ;
        trg.fx(r,tsim) $ OMN(r) = trg.l(r,tsim) + (ygov0(r,"ctax") * (ygov.l(r,"ctax",tsim-1) - ygov_bau(r,"ctax",tsim-1))) ;        
    );
) ;
...
if((Varflag ge 103), 
    ...
    IfCalMacro(r,"power mix") $ EGY(r)  = 0  ;

    if(VarFlag eq 103, IfCalMacro(r,"power mix") $ OMN(r) = 0; );
    if(VarFlag eq 104, IfCalMacro(r,"power mix") $ OMN(r) = 1; );
    if(VarFlag eq 105, IfCalMacro(r,"power mix") $ OMN(r) = 1; );
...
$offtext
*********************************************************************************************************
**Actual calibration ElyMix calibration

$ifi %simName%=="NoShk"     ElyMixCalSim(r,powa,tsim) $ OMN(r)= ElyMixCal(r,powa,"CurrentPol",tsim)  ;
$ifi %simName%=="Sim01"     ElyMixCalSim(r,powa,tsim) $ OMN(r)= ElyMixCal(r,powa,"CurrentPol",tsim)  ;
$ifi %simName%=="Sim02"     ElyMixCalSim(r,powa,tsim) $ OMN(r)= ElyMixCal(r,powa,"CurrentPol",tsim)  ;
$ifi %simName%=="Sim03"     ElyMixCalSim(r,powa,tsim) $ OMN(r)= ElyMixCal(r,powa,"CurrentPol",tsim)  ;
$ifi %simName%=="Sim04"     ElyMixCalSim(r,powa,tsim) $ OMN(r)= ElyMixCal(r,powa,"MidNDC",tsim)        ;
$ifi %simName%=="Sim05"     ElyMixCalSim(r,powa,tsim) $ OMN(r)= ElyMixCal(r,powa,"NDCs",tsim)        ;
*$ifi %simName%=="Sim05"     ElyMixCalSim(r,powa,tsim) $ OMN(r)= ElyMixCal(r,powa,"MidPoint",tsim)        ;


$ifi %simName%=="Sim03"       ElyMixCalSim(r,powa,tsim) $ (not allMCD(r))= ElyMixCal(r,powa,"NDCs",tsim) 
$ifi %simName%=="Sim04"       ElyMixCalSim(r,powa,tsim) $ (not allMCD(r))= ElyMixCal(r,powa,"NDCs",tsim)        ;

*$ifi %simName%=="SimNZrow"     ElyMixCalSim(r,powa,tsim) $ (not allMCD(r))= ElyMixCal(r,powa,"NetZero2050",tsim)  ;
*$ifi %simName%=="SimNZ"        ElyMixCalSim(r,powa,tsim) $ (not allMCD(r))= ElyMixCal(r,powa,"NetZero2050",tsim) ;
*$ifi %simName%=="SimDTrow"     ElyMixCalSim(r,powa,tsim) $ (not allMCD(r))= ElyMixCal(r,powa,"DelayTran",tsim)  ;

*** ElyMixCalSim(r,powa,tsim) $ ((allMCD(r) and (not OMN(r)))) = ElyMixCal(r,powa,"NDCs",tsim) ;
...
*HRR if YrFinTgt=YearEndofSim    
    if(year le %YrFinTgt%, 
* if not equal then:
*    if(year le %YearEndofSim%, 
        IfMcpCapEq  = 1 ;
...
* CBAM
$ontext
if( (VarFlag ge 41) and (VarFlag lt 100), 

...
if(year eq 2024, 
* Electricity is subsidized only for households
    paTax.fx(r,"p_c-c",aa,tsim) $ (OMN(r) and (paTax.l(r,"p_c-c",aa,tsim) lt 0)) = paTax.l(r,"p_c-c",aa,tsim) * 6.5 ; 
    paTax.fx(r,"ely-c","hhd",tsim) $ (OMN(r) ) = paTax.l(r,"ely-c","hhd",tsim) * (-122.5) ; !! 
*    paTax.fx(r,"ely-c",aa,tsim) $ (OMN(r) and (paTax.l(r,"ely-c",aa,tsim) lt 0)) = paTax.l(r,"ely-c",aa,tsim) * (+60) ; !! 

);

if(year ge 2025,
    paTax.fx(r,"ely-c",aa,tsim) $ (OMN(r) and paTax.l(r,"ely-c",aa,tsim)) = paTax.l(r,"ely-c",aa,tsim-1) ; 
    paTax.fx(r,"p_c-c",aa,tsim) $ (OMN(r) and paTax.l(r,"p_c-c",aa,tsim)) = paTax.l(r,"p_c-c",aa,tsim-1) ; 
);
...
IF(VarFlag ge 1,
    if(year ge 2029,   xpFlag(r,"olp-a") $ EGY(r) = 0 ;    ) ;
    if(year ge 2040,   tfp_xpv.fx(r,"coa-a",v,tsim) $ KGZ(r)= tfp_xpv.l(r,"coa-a",v,tsim-1) * (1 - 0.1) ;    ) ;
);

if(Varflag eq 105, 
    if(year ge 2028, xpFlag(r,"olp-a") $ OMN(r) = 0 ;    ) ;
    if(year eq 2034, 

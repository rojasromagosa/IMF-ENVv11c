...
* to model, but this element "ely" is not mapped with i0
    ...
    a  "Original activities (included cgds)" /
        ...
        $$batinclude "%sDir%\sAgg%SectorAgg%.gms" "define_act" "%IfPower%"
...
* Mapping of GTAP sectors "a0" to Model sectors "a"
    ...
    mapa(a0,a) "Mapping of GTAP to Model sectors" /
        $$batinclude "%sDir%\sAgg%SectorAgg%.gms" "define_mapa"
    /
...
* Only one electricity good for this project --> %IfElyGoodDesag%=="OFF"
    ...
    commf "Model commodities" /
        ...
        $$batinclude "%sDir%\sAgg%SectorAgg%.gms" "define_act" "%IfElyGoodDesag%"
...
*SET mapaf(i,actf) "Mapping from original to modeled activities" /
*        $$batinclude "%sDir%\sAgg%SectorAgg%.gms" "define_mapaf"
*    /
*;
...
SET mapIF(i,commf) "Mapping from original to modeled commodities" /
        $$batinclude "%sDir%\sAgg%SectorAgg%.gms" "define_mapif"
/;
$OffText
...
SETS
    ...
    mapActSort(sortOrder,actf) /
        $$batinclude "%sDir%\sAgg%SectorAgg%.gms" "define_mapActSort"
    /
    mapCommSort(sortOrder,commf) /
        $$batinclude "%sDir%\sAgg%SectorAgg%.gms" "define_mapCommSort"
    /
;
...
*    fisherya(actf)  "Fisheries sector" / fsh "Fisheries" /
    fisherya(actf)  "Fisheries sector" /  /
...
$offtext
    ...
    frta(actf)      "Fertilizer/Chemicals"    / /
    cementa(actf)   "Non-metallic minerals"   / /
    PPPa(actf)      "Paper & Paper Products"  / /
    I_Sa(actf)      "Iron and Steel"          / /
    ELEa(actf)      "Electronic sectors"      / /
    FMPa(actf)      "Fabricated metal sectors"/ /
    wooda(actf)     "Wood sector" / /
    MTEa(actf)      "Manuf. of transport equipement and vehicles" / /
    NFMa(actf)      "Non-Ferrous Metals"      / /
    omana(actf)     "Other manufacturing sectors"    /        /
    FDPa(actf) "Food products sectors" /        /
    TXTa(actf) "Textiles sectors" /        /
...
$ontext
    ...
    /
$offtext
    frti(commf) "Fertilizer commodities" / /
    feedi(commf) "Feed commodities" / /
...
* Other Manufacturing:
    ...
    omani(commf)     "Other Manufacturing commodities"    /
        ...
        /
$offText
    Fisheryi(commf)  "Fisheries sector" / /

* Energy Intensive Industries:

    cementi(commf) "Non-metallic minerals"   / /
    PPPi(commf)    "Paper & Paper Products"  / /
    I_Si(commf)    "Iron and Steel"          / /

* Other Manufacturing:

    ELEi(commf)  "Electronic goods"         / /
    FMPi(commf)  "Fabricated metal products"/ /
    woodi(commf) "Wood products" /        /
    MTEi(commf)  "Transport equipement and vehicles commodities" /    /
    NFMi(commf) "Non-Ferrous Metals" / /
    omani(commf)     "Other Manufacturing commodities"    /
    oma   "Other manufacturing"
        /
...
*    $$batinclude "%sDir%\sAgg%SectorAgg%.gms" "define_mapia" / ;
*/ ;
...
*set mapaga(aga,actf) "mapping of individual sector to aggregate sector" /
*    $$batinclude "%sDir%\sAgg%SectorAgg%.gms" "define_mapaga"
*/ ;
...
*        $$IF %split_acr%=="OFF" cro "Crop bundle"
        $$IF %split_acr%=="OFF" agr "Crop bundle"
